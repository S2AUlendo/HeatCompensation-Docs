...
# Import the rotor part
from pathlib import Path
dir_path = os.path.dirname(os.path.realpath(__file__))
current_dir = Path(__file__).resolve() # Gets the absolute path of the current file
grandparent_dir = current_dir.parents[3] # Accesses the grandparent directory using index 2
...
sdf_sgt0 = zoner.add_segment(zone_type=zoner.PartZoneType.SDF,
                             color=(255, 0, 0))
us_sgt0 = zoner.add_segment(zone_type=zoner.PartZoneType.UPSKIN,
                            color=(0, 255, 0))
ds_sgt0 = zoner.add_segment(zone_type=zoner.PartZoneType.DOWNSKIN,
                            color=(0, 0, 255))
core_sgt0 = zoner.add_segment(zone_type=zoner.PartZoneType.CORE,
                              color=(140, 140, 140))
support_sgt0 = zoner.add_segment(zone_type=zoner.PartZoneType.CORE,
                                 color=(140, 140, 140))
...
part_strat = zoner.new_segmentation_strategy(core_segment=core_sgt0,
                                             volumetric_segments=[(us_sgt0, 4),
                                                                  (ds_sgt0, 4),
                                                                  (sdf_sgt0, 4)])
...
part_schema = toolpather.new_toolpath_schema(segmentation_strategy=part_strat,
                                             contour_strategy=cst0)
support_schema = toolpather.new_toolpath_schema(segmentation_strategy=support_strat,
                                                contour_strategy=cst0)
